...
to go
  ask ants [
    ...
    rotate-ant
    if patch-ahead ant-vel = nobody [
      rt random 360
    ]
    forward ant-vel
    put-pheromone
  ]
  diffuse ph-n diffusion-coef
  diffuse ph-f diffusion-coef
  ask patches [
    set ph-n evaporation-coef * ph-n
    set ph-f evaporation-coef * ph-f
    if ph-n < 0.001 [
      set ph-n 0
    ]
    if ph-f < 0.001 [
